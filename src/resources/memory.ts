...
export class MemoryResource {
  ...
  async add(storeKey: string, data: AddMemoryRequest): Promise<MemoryItem> {
    const response = await this.http.request<{ memory: MemoryItem }>(
      'POST',
      `/api/client/v1/memory/stores/${storeKey}/memories`,
      {
        body: data,
      },
    );
...
export class MemoryStoresResource {
  ...
  async create(data: CreateMemoryStoreRequest): Promise<MemoryStore> {
    const response = await this.http.request<{ store: MemoryStore }>('POST', '/api/client/v1/memory/stores', {
      body: data,
    });
  ...
  async get(storeKey: string): Promise<MemoryStore> {
    const response = await this.http.request<{ store: MemoryStore }>(
      'GET',
      `/api/client/v1/memory/stores/${storeKey}`,
    );
  ...
  async update(storeKey: string, data: UpdateMemoryStoreRequest): Promise<MemoryStore> {
    const response = await this.http.request<{ store: MemoryStore }>('PATCH', `/api/client/v1/memory/stores/${storeKey}`, {
      body: data,
    });
