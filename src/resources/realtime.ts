...
export interface RealtimeConnectOptions {
  /**
   * Realtime model or chat model key. The response generator is a
   * session-start choice — it may alternatively be set via `updateSession`
   * before the first response, but locks once the conversation starts.
   */
  model?: string;
  /** Agent key — responses are generated by this agent instead of a raw model. Locked like `model`. */
  agent?: string;
  /**
   * Downstream auth/data for agent tool calls — sent as a `session.update`
   * right after the socket opens. Also settable/refreshable later via
   * `updateSession({ runtime_context })`.
   */
...
export class RealtimeResource {
  ...
  url(options?: { model?: string; agent?: string }): string {
  ...
  async connect(options?: RealtimeConnectOptions): Promise<RealtimeConnection> {
    ...
    const socket = new Ctor(this.url({ model: options?.model, agent: options?.agent }));
    ...
    if (options?.runtimeContext) {
      connection.updateSession({ runtime_context: options.runtimeContext });
    }
    return connection;
  }
}
